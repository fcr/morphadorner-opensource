...
public class Sentence {
     ...
     */
    public ArrayList<Word>word = new ArrayList<Word>();
    /**
    ...
    public boolean separate_sentence(String s, ParseOptions opts) {
        ...
        return (word.size() > (dict.left_wall_defined ? 1 : 0) + (dict.right_wall_defined ? 1 : 0));
    }
    ...
    private boolean issue_sentence_word(String s) {
        ...
        if (word.size() == GlobalBean.MAX_SENTENCE) {
            throw new RuntimeException(". The sentence has too many words.\n");
        }
        word.add(new Word(s,dict));
        ...
        word.get(word.size()-1).firstupper = Character.isUpperCase(s.charAt(0));
    ...
    public void build_sentence_expressions(ParseOptions opts) {
        ...
        for (i = 0; i < word.size(); i++) {
            if (!(i == first_word || (i > 0 && ":".equals(word.get(i-1).string)) || post_quote[i] == 1))
                continue;
            s = word.get(i).string;
            if (Character.isUpperCase(s.charAt(0))) {
                ...
                if (dict.boolean_dictionary_lookup(u)) {
                    ...
                    if (dict.boolean_dictionary_lookup(s)) {
                        ...
                        word.get(i).x = XNode.catenate_XNodes(word.get(i).x, e);
                    } else {
                        /* If the upper-case version isn't there, replace the u.c. disjuncts with l.c. ones */
                        temp_word = new StringBuffer(s);
                        temp_word.setCharAt(0, Character.toLowerCase(temp_word.charAt(0)));
                        word.get(i).string = temp_word.toString();
                        e = Word.build_word_expressions(word.get(i).string,this.dict);
                        word.get(i).x = e;
                    }
                }
            }
     ...
     */
     public void set_is_conjunction() {
        ...
        for (int w = 0 ; w<word.size() ;w++) {
            s = word.get(w).string;
            word.get(w).is_conjunction = ((s.equals("and")) || (s.equals("or")) || (s.equals("but")) || (s.equals("nor")));
        }
    }
    ...
    public void prepare_to_parse(ParseOptions opts) {
        ...
        for (i = 0; i < word.size(); i++) {
            word.get(i).d = Disjunct.eliminate_duplicate_disjuncts(opts, word.get(i).d);
        }
    ...
    private void checkDuplicate(String label) {
        ...
        for (int i = 0; i < word.size(); i++) {
            for (Disjunct d = word.get(i).d; d != null; d = d.next) {
                if (disj.contains(d)) {
                    throw new RuntimeException(label + " dup disj w=" + word.get(i) + " d=" + d.string);
                } else {
                    disj.add(d);
                    for (Connector c = d.left; c != null; c = c.next) {
                        if (conn.contains(c)) {
                            throw new RuntimeException(
                                label + " dup conn w=" + word.get(i) + " d=" + d.string + " c=" + c.string + "-");
                        } else {
                            conn.add(c);
                        }
                    }
    ...
    public void conjunction_prune(ParseOptions opts) {
        ...
        /* we begin by unmarking all disjuncts.  This would not be necessary if
           ...
           */
        for (int w = 0; w < word.size(); w++) {
            for (d = word.get(w).d; d != null; d = d.next) {
                d.marked = false;
            }
        }
        ...
        if ((d.left == null) && region_valid(0, sent.word.size(), d.right, null)) {
            mark_region(0, sent.word.size(), d.right, null);
            d.marked = true;
        }
        }
        mark_region(0, sent.word.size(), null, null);
        */

        if (null_links) {
            mark_region(-1, word.size(), null, null);
        } else {
            for (int w = 0; w < word.size(); w++) {
                ...
                if (deletable[0][w]) {
                    for (d = word.get(w).d; d != null; d = d.next) {
                        if ((d.left == null) && region_valid(w, word.size(), d.right, null) > 0) {
                            mark_region(w, word.size(), d.right, null);
                            d.marked = true;
                        }
                    }
                }
    ...
    void install_fat_connectors() {
        ...
        /* sentence */
        for (int i = 0; i < word.size(); i++) {
            if (word.get(i).is_conjunction) {
                word.get(i).d = Disjunct.catenate_disjuncts(word.get(i).d, build_AND_disjunct_list(word.get(i).string));
            } else {
                word.get(i).d = Disjunct.catenate_disjuncts(word.get(i).d, explode_disjunct_list(word.get(i).d));
                if (word.get(i).string.equals(",")) {
                    word.get(i).d = Disjunct.catenate_disjuncts(word.get(i).d, build_COMMA_disjunct_list());
                }
            }
        }
    }
    ...
    public void compute_matchers_for_a_label(int k) {
        /* This takes a label k, modifies the list of disjuncts with that
           ...
           The only hard part is finding the word.size() of each of the strings
    ...
    public void print_AND_statistics(ParseOptions opts) {
        ...
        if (and_data.LT_size != 0) {
            opts.out.println("average list word.size(): " + ((float)GlobalBean.STAT_N_disjuncts / and_data.LT_size));
        }
    ...
    public void build_effective_dist(boolean has_conjunction) {
        /*
           ...
           link word.size(), then that connector cannot be satisfied by linking these
           two words.
        ...
        int i, j, diff;
        effective_dist = new int[word.size()][];
        for (i = 0; i < word.size(); i++) {
            effective_dist[i] = new int[word.size() + 1];
        }
        for (i = 0; i < word.size(); i++) {
        ...
        if (null_links) {
            ...
            for (diff = 1; diff < word.size(); diff++) {
                for (i = 0; i + diff <= word.size(); i++) {
                    j = i + diff;
                    if (deletable[i + 1][j]) {
            ...
            /* now when you link to a conjunction, your effective word.size() is 1 */
            for (i = 0; i < word.size(); i++) {
                for (j = i + 1; j < word.size(); j++) {
                    if (word.get(i).is_conjunction || word.get(j).is_conjunction)
                        effective_dist[i][j] = 1;
                }
            }
        }
    ...
    public void build_deletable(boolean has_conjunction) {
        ...
        deletable = new boolean[word.size() + 1][];
        for (i = -1; i < word.size(); i++) {
            deletable[i + 1] = new boolean[word.size() + 1];
            ...
            for (j = 0; j <= word.size(); j++) {
                if (j == i + 1) {
                    ...
                    j > i + 2
                        && (word.get(i+1).is_conjunction
                            || word.get(j-1).is_conjunction
                            || (word.get(i+1).string.equals(",") && conj_in_range(i + 2, j - 1))
                            || (word.get(j-1).string.equals(",") && conj_in_range(j, word.size() - 1)))) {
                    deletable[i + 1][j] = true;
                } else if (j > i) {
                    for (k = i + 1; k < j; k++) {
                        if ("either".equals(word.get(k).string)
                            || "neither".equals(word.get(k).string)
                            || "both".equals(word.get(k).string)
                            || "not".equals(word.get(k).string))
                            continue;
                        if (("only".equals(word.get(k).string)) && (k > i + 1) && ("not".equals(word.get(k-1).string)))
                            continue;
                        break;
                    }
                    deletable[i + 1][j] = (k == j);
    ...
    private void set_connector_length_limits(ParseOptions opts) {
        ...
        for (i = 0; i < word.size(); i++) {
            for (d = word.get(i).d; d != null; d = d.next) {
    ...
    public int parse(int cost, ParseOptions opts) {
        ...
        total = count(-1, word.size(), null, null, cost + 1, opts);
        if (opts.verbosity > 1) {
    ...
    public void init_table() {
        ...
        if (word.size() >= 10) {
            ctable_size = (1 << 16);
            /*  } else if (sent.word.size() >= 10) {
                table_size = (1 << (((6*(sent.word.size()-10))/30) + 10)); */
        } else if (word.size() >= 4) {
            ctable_size = (1 << (((6 * (word.size() - 4)) / 6) + 4));
        } else {
            ctable_size = (1 << 4);
        }
    ...
    public void init_fast_matcher() {
        ...
        match_cost = 0;
        for (w = 0; w < word.size(); w++) {
            len = left_disjunct_list_length(word.get(w).d);
            ...
            len = right_disjunct_list_length(word.get(w).d);
            size = MyRandom.next_power_of_two_up(len);
    ...
    public boolean build_parse_set(int cost, ParseOptions opts) {
        ParseSet whole_set;
        whole_set = parse_info.parse_set(this, null, null, -1, word.size(), null, null, cost + 1, opts);
    ...
    public void build_sentence_disjuncts(ParseOptions opts, int cost_cutoff) {
        ...
        */
        ...
        for (w = 0; w < word.size(); w++) {
            d = null;
            for (x = word.get(w).x; x != null; x = x.next) {
    ...
    public void print_disjunct_counts(ParseOptions opts) {
        ...
        for (i = 0; i < word.size(); i++) {
            ...
            opts.out.print(word.get(i).string + "(" + c + ") ");
        }
        opts.out.println();
        opts.out.println();
    }
    ...
    public void post_process_linkages(ParseOptions opts) {
        ...
        if (overflowed) {
            ...
            MyRandom.my_random_initialize(N_linkages_found + word.size());
            for (in = 0; in < N_linkages_alloced; in++) {
        ...
        if (word.size() >= opts.twopass_length) {
            for (in = 0; in < N_linkages_alloced; in++) {
    ...
    public PPNode post_process(Postprocessor pp, ParseOptions opts, Sublinkage sublinkage, boolean cleanup) {
        ...
        if (q_pruned_rules == false && word.size() >= opts.twopass_length)
    ...
    private void and_dfs_commas(int w) {
        ...
        for (lol = ParseInfo.word_links[w]; lol != null; lol = lol.next) {
            if (lol.dir == 1) {
                ...
                if (word.get(lol.word).string.equals(",")) {
                    /* pointing to a comma */
    ...
    */
    ...
    public void print_expression_sizes(ParseOptions opts) {
        ...
        for (w = 0; w < word.size(); w++) {
            ...
            opts.out.print(word.get(w).string + "[" + size + "] ");
        }
        opts.out.println();
        opts.out.println();
    }
    ...
    public static String intersect_strings(String s, String t) {
        ...
        if (j > i) {
            ...
            len = i;
        } /* s is now the longer (at least not the shorter) string */ /* and len is its word.size() */
  ...
  private void construct_comma() {
        ...
        */
        int w;
        for (w = 0; w < word.size() - 1; w++) {
            if ((word.get(w).string.equals(",")) && word.get(w+1).is_conjunction) {
                word.get(w).d = Disjunct.catenate_disjuncts(special_disjunct(GlobalBean.COMMA_LABEL, '+', "", ","), word.get(w).d);
                word.get(w+1).d = glom_comma_connector(word.get(w+1).d);
            }
        }
    }
    ...
    */
    private void construct_either() {
        ...
        if (!sentence_contains("either"))
            return;
        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("either"))
                continue;
            word.get(w).d =
                Disjunct.catenate_disjuncts(special_disjunct(GlobalBean.EITHER_LABEL, '+', "", "either"), word.get(w).d);
        }

        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("or"))
                continue;
            word.get(w).d = glom_aux_connector(word.get(w).d, GlobalBean.EITHER_LABEL, false);
        }
    }
    ...
    private void construct_neither() {
        ...
        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("neither"))
                continue;
            word.get(w).d =
                Disjunct.catenate_disjuncts(special_disjunct(GlobalBean.NEITHER_LABEL, '+', "", "neither"), word.get(w).d);
        }

        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("nor"))
                continue;
            word.get(w).d = glom_aux_connector(word.get(w).d, GlobalBean.NEITHER_LABEL, true);
        }
    }
    ...
    private void construct_notonlybut() {
        ...
        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("not"))
                continue;
            word.get(w).d = Disjunct.catenate_disjuncts(special_disjunct(GlobalBean.NOT_LABEL, '+', "", "not"), word.get(w).d);
            if (w < word.size() - 1 && word.get(w+1).string.equals("only")) {
                word.get(w+1).d =
                    Disjunct.catenate_disjuncts(special_disjunct(GlobalBean.NOTONLY_LABEL, '-', "", "only"), word.get(w+1).d);
                d = special_disjunct(GlobalBean.NOTONLY_LABEL, '+', "", "not");
                d = add_one_connector(GlobalBean.NOT_LABEL, '+', "", d);
                word.get(w).d = Disjunct.catenate_disjuncts(d, word.get(w).d);
            }
        }
         ...
         */
        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("but"))
                continue;
            word.get(w).d = glom_aux_connector(word.get(w).d, GlobalBean.NOT_LABEL, false);
    ...
    private void construct_both() {
        ...
        if (!sentence_contains("both"))
            return;
        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("both"))
                continue;
            word.get(w).d = Disjunct.catenate_disjuncts(special_disjunct(GlobalBean.BOTH_LABEL, '+', "", "both"), word.get(w).d);
        }

        for (w = 0; w < word.size(); w++) {
            if (!word.get(w).string.equals("and"))
                continue;
            word.get(w).d = glom_aux_connector(word.get(w).d, GlobalBean.BOTH_LABEL, false);
        }
    }
    ...
    */
    ...
    public void delete_unmarked_disjuncts() {
        ...
        for (w = 0; w < word.size(); w++) {
            d_head = null;
            for (d = word.get(w).d; d != null; d = dx) {
                dx = d.next;
                if (d.marked) {
                    d.next = d_head;
                    d_head = d;
    ...
    */
    ...
    public void clean_up(int w) {
        ...
        */
        ...
        word.get(w).d = head_disjunct.next;
    }
    ...
    public int count_disjuncts_in_sentence() {
        ...
        count = 0;
        for (w = 0; w < word.size(); w++) {
            count += Disjunct.count_disjuncts(word.get(w).d);
        }
        return count;
    }
 ...
 *      deletable, this is equivalent to RUTHLESS.   --DS, 7/97
        ...
        while (true) {
            ...
            for (w = word.size() - 1; w >= 0; w--) {
                for (d = word.get(w).d; d != null; d = d.next) {
                    if (d.right == null)
                        continue;
                    if (right_connector_list_update(d.right, w, w, true) >= word.size()) {
                        for (c = d.right; c != null; c = c.next)
                            c.word = GlobalBean.BAD_WORD;
                        for (c = d.left; c != null; c = c.next)
                            c.word = GlobalBean.BAD_WORD;
    ...
    * take this connector list, and try to match it with the words
    ...
    public int right_connector_list_update(Connector c, int word_c, int w, boolean shallow) {
        ...
        foundmatch = false;
        for (;(n < word.size()) && ((n - w) <= GlobalBean.MAX_SENTENCE); n++) {
            power_cost++;
    ...
    public void init_power() {
        ...
        for (w = 0; w < word.size(); w++) {
            head = null;
            for (d = word.get(w).d; d != null; d = xd) {
                xd = d.next;
                if ((set_dist_fields(d.left, w, -1) < 0) || (set_dist_fields(d.right, w, 1) >= word.size())) {
                    d.next = null;
                } else {
                    d.next = head;
                    head = d;
        ...
        for (w = 0; w < word.size(); w++) {
            len = left_connector_count(word.get(w).d);
            size = MyRandom.next_power_of_two_up(len);
            ...
            len = right_connector_count(word.get(w).d);
            size = MyRandom.next_power_of_two_up(len);
