...
public class Linkage {
    ...
    void compute_chosen_words(Sentence sent) {
        ...
        for (i = 0; i < sent.word.size(); i++) { /* get rid of those ugly ".Ixx" */
            chosen_words[i] = sent.word.get(i).string;
        ...
        if (sent.dict.right_wall_defined) {
            chosen_words[sent.size() - 1] = GlobalBean.RIGHT_WALL_DISPLAY;
        }
        for (i = 0; i < sent.size(); ++i) {
            word.add(chosen_words[i]);
        }
    }
    ...
    public String linkage_print_diagram() {
        ...
        for (; k < N_words_to_print; k++) {
            ttt = 0;
            i = 0;
            while (ttt < word.get(k).length()) {
                xpicture[0][sss] = word.get(k).charAt(ttt);
                sss++;
                ttt++;
                i++;
            }
        ...
        while (i < N_words_to_print) {
            ...
            width = 0;
            do {
                width += word.get(i).length() + 1;
                i++;
            } while ((i < N_words_to_print) && (word.get(i).length() + 1) + 1 < x_screen_width);
    ...
    public void adjust_subordinate_clauses(int numcon_total, int numcon_subl) {
        ...
        for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
            if (Postprocessor.post_process_match("MVs", constituent[c].start_link)
                ...
                if (word.get(constituent[c].left).equals(","))
                    constituent[c].left++;
            }
        }
    }
    ...
    private void adjust_for_right_comma(int c) {
        ...
        if ((word.get(constituent[c].right).equals(",")) || (word.get(constituent[c].right).equals("RIGHT-WALL"))) {
            w--;
            while (true) {
                if (word_used[current][w] == 1)
                    break;
    ...
    private int read_constituents_from_domains(int numcon_total, int s) {
        ...
        for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
            if ((constituent[c].domain_type == 'p') && (word.get(constituent[c].left).equals(","))) {
                constituent[c].left++;
            }
        }
            ...
            for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
                for (c2 = numcon_total; c2 < numcon_total + numcon_subl; c2++) {
                    if ((constituent[c].left < constituent[c2].left)
                        ...
                        if ((word.get(constituent[c2].right).equals(","))
                            || (word.get(constituent[c2].right).equals("RIGHT-WALL"))) {
                            if (opts.verbosity >= 2)
                                opts.out.println("Adjusting " + c2 + " to fix comma overlap");
                            adjust_for_right_comma(c2);
                            adjustment_made = 1;
                        } else if (word.get(constituent[c].left).equals(",")) {
                            if (opts.verbosity >= 2)
    ...
    private int last_minute_fixes(int numcon_total) {
        ...
        for (c = 0; c < numcon_total; c++) {
            ...
            if ((constituent[c].domain_type == 'h') && (word.get(constituent[c].left - 1).equals("$"))) {
                constituent[c].left--;
            }
        ...
        for (c = 0; c < numcon_total; c++) {
            if ((constituent[c].right == (num_words) - 3)
                ...
                && (sent.word.get((num_words) - 2).string.equals(".")))
                constituent[c].right++;
        }
